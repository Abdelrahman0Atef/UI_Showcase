...
class CategoriesProductViewModel {
  final GenericCubit<List<ProModel>> productsCubit =
      GenericCubit<List<ProModel>>([]);
  ...
  Future<void> getProductsByCategory(String category) async {
    try {
      final data = await restApiServices.get(
        '${MyStrings.baseUrl}/${MyStrings.apiCategory}/$category',
      );
      final filteredProducts =
          (data as List).map((e) => ProModel.fromJson(e)).toList();
      productsCubit.onUpdateData(filteredProducts);
    } catch (e) {
      try {
        final allData = await restApiServices.get(MyStrings.baseUrl);
        final allProducts =
            (allData as List).map((e) => ProModel.fromJson(e)).toList();
        final filtered =
            allProducts
                .where(
                  (product) =>
